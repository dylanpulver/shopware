...
async function mountHost({ blockContent = '<div class="inner">content</div>', overrides = '', legacyShim = true } = {}) {
    ...
    const shimAttribute = legacyShim ? '' : ':sw-internal-legacy-shim="false"';
    ...
    return mount(
        {
            ...
            components: {
                ...
                'owner-component': {
                    ...
                    template: `<section><sw-block name="inspected-block" ${shimAttribute}>${blockContent}</sw-block></section>`,
                },
            },
        },
        {
...
describe('sw-block with the block inspector enabled', () => {
    ...
    it('leaves the registration of generated Twig wrappers to the template factory', async () => {
        wrapper = await mountHost({ legacyShim: false });
