...
describe('core/factory/template.factory.js - block inspector markers', () => {
    ...
    it('keeps the marker on the content inside a generated native extension point', () => {
        registerNativeExtensionTargets({ component: 'bi-native', blocks: ['bi_native_inner'] });
        TemplateFactory.registerComponentTemplate(
            'bi-native',
            '{% block bi_native_outer %}{% block bi_native_inner %}<p>x</p>{% endblock %}{% endblock %}',
        );

        // The wrapper is transparent: marking it would fall through and overwrite the inner marker.
        expect(renderedHtml('bi-native')).toBe(
            '<sw-block name="bi_native_inner" :data="$dataScope" :sw-internal-legacy-shim="false">' +
                '<p data-sw-block="bi_native_inner bi_native_outer">x</p></sw-block>',
        );
        expect(getInspectedBlock('bi_native_inner')?.kind).toBe('twig');
    });
