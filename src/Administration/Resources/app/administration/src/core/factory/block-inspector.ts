 ...
 *
 * `<template>` is Vue's grouping tag for `v-if`, `v-for` and named slots. `<sw-block>` is the
 * extension point wrapper the template factory inserts: marking it would fall through to its root
 * and overwrite the marker of the block it hosts, so the marker goes onto its content instead.
 */
